...
МЕТАЛЛ — считай площадь м² ТОЛЬКО для этих типов (всё остальное игнорируй):
...
ТОЛЩИНА металла: берём из текста (б=0.5, б=0.7 и т.д.)
Если не указано: любая сторона > 400мм -> 0.7мм, иначе -> 0.5мм

ВАЖНО — УМНОЖЕНИЕ НА КОЛИЧЕСТВО СИСТЕМ:
Если внизу страницы есть сноска со звёздочкой (*) вида:
"* спецификация посчитана на одну систему" —
тогда найди сколько систем указано в названии раздела (например "ВЕ8,9,11" = 3 системы)
и умножь ВСЁ в этом разделе на это количество:
- все воздуховоды (qty и area_m2)
- все переходы, отводы, тройники (qty и area_m2)
- всю изоляцию (qty)
- листовую сталь (area_m2)
Если такой сноски НЕТ — не умножай ничего, даже если в названии несколько систем через запятую.
...
def _call_claude(content: list) -> str:
    ...
    logging.info(
        f"PDF обработан | Токены входящие: {data.get('usage', {}).get('input_tokens', '?')} | Исходящие: {data.get('usage', {}).get('output_tokens', '?')} | Stop: {data.get('stop_reason', '?')}")
    return text
